...
import { updatePersonalities } from "../graphql/mutations";
const client = generateClient();
export default function PersonalitiesUpdateForm(props) {
  const {
    id: idProp,
    personalities: personalitiesModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = personalitiesRecord
      ? { ...initialValues, ...personalitiesRecord }
      : initialValues;
  ...
  const [personalitiesRecord, setPersonalitiesRecord] = React.useState(
    personalitiesModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getPersonalities.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getPersonalities
        : personalitiesModelProp;
      setPersonalitiesRecord(record);
    };
    queryData();
  }, [idProp, personalitiesModelProp]);
  React.useEffect(resetStateValues, [personalitiesRecord]);
  const validations = {
    name_1: [],
    personality_1: [],
    name_2: [],
    ...
    >
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || personalitiesModelProp)}
