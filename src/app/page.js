...
export function Home({ signOut, user }) {
  ...
  // Subscribe to Personalities updates so fresh avatar URLs (image_1/image_2)
  // generated by the trigger Lambda land in state without a page reload.
  React.useEffect(() => {
    const sub = amplifyClient.graphql({
      query: subscriptions.onUpdatePersonalities,
      authMode: 'userPool',
    }).subscribe({
      next: ({ data }) => setPersonalities(data.onUpdatePersonalities),
      error: (err) => console.log("Personalities subscription error: ", err),
    });
    return () => sub.unsubscribe();
  }, []);
