...
5. The Amplify PR preview *should* build automatically, but **PR previews are currently unreliable** — the GitHub App webhook does not consistently fire for PRs opened by `danrohtbart-bot`. If `pr-{N}` does not appear in Amplify within 5 minutes of opening the PR, do not block on it. Proceed to merge once unit tests pass and the logic has been manually verified. See the "PR preview" pitfall below.
6. If a preview does build, assess it at `https://pr-{N}.d3bo8xtge7s7fh.amplifyapp.com` — log in with `.env.test` credentials (`TEST_USER_EMAIL` / `TEST_USER_PASSWORD`).
...
- **GitHub token:** Claude Code sessions use the `danrohtbart-bot` GitHub account via a **classic PAT** (`GITHUB_TOKEN` env var) stored in `.claude/settings.local.json` (gitignored). The bot account has Write collaborator access to the repo. Classic PAT was necessary because fine-grained PATs from a collaborator cannot create PRs on repos owned by a different user. The token has `repo` scope. The bot opens PRs; Dan approves and merges to `main`. Claude **cannot** merge to `main` — the branch protection rule requires Dan's review, and the bot is not the repo owner.
...
- **Playwright version** must be consistent in three places: `package.json`, the Docker image tag in `amplify.yml`, and the `npm install @playwright/test@X.Y.Z` line. They must all match. Current version: **1.59.1**.
...
- **Amplify PR previews are unreliable for this repo.** The repo is public, and Amplify disabled PR previews when it went public. `enablePullRequestPreview` was manually re-enabled via CLI (`aws amplify update-branch --enable-pull-request-preview`), but the GitHub App webhook for `pull_request` events does not fire consistently for PRs opened by `danrohtbart-bot`. If a preview does not appear within 5 minutes, skip it and rely on unit tests + manual verification on dev after merge.
- **`amplify push` wipes any Lambda environment variable not declared in the CloudFormation template.** Never set secrets directly in the Lambda console and expect them to persist — they will be overwritten on the next push. The correct pattern is to store the secret in SSM Parameter Store and read it at Lambda runtime (see the avatar pipeline below).
- **All Bedrock image generation models are blocked in this account.** Titan Image Generator v1/v2 are end-of-life. Nova Canvas returns a "Legacy / not actively used in 30 days" error. Claude Haiku and other text-only models cannot generate images. **Do not attempt to use any Bedrock image model** — they will all fail silently in the Lambda's try/catch.
...
When a Personalities record is saved by the webapp, the Lambda generates a caricature avatar for each personality slot whose text changed.

### How it works

1. **Llama3 via Bedrock** (`meta.llama3-70b-instruct-v1:0`) — given the personality text and bot name, writes a detailed caricature image-generation prompt describing what physical feature to exaggerate.
2. **DALL-E 2 via OpenAI API** (`dall-e-2`, 256×256) — generates the actual PNG image from the prompt.
3. **S3** — the raw PNG is uploaded to `botchat-avatars-dev-253178317163` (dev) under `avatars/`. The object is publicly readable.
4. **DynamoDB** — the S3 HTTPS URL is stored in `image_1` / `image_2` on the Personalities record (not base64 — DynamoDB's 400KB limit can't hold DALL-E images as base64).
5. **Frontend** — `page.js` calls `avatarForSpeaker()` which reads `image_1`/`image_2` from the loaded Personalities record and uses the value directly as `<img src>`.

The loop guard in the Lambda prevents re-triggering when the Lambda's own image write echoes back through the DynamoDB stream.

### Secrets and infrastructure that must exist

| Resource | Location | Notes |
|----------|----------|-------|
| OpenAI API key | SSM Parameter Store: `/botchat/openai-api-key` (SecureString) | Read at Lambda cold start via `getOpenAiKey()`. **Do not set it as a raw Lambda env var** — `amplify push` will wipe it. |
| SSM path env var | `OPENAI_API_KEY_SSM_PATH=/botchat/openai-api-key` | Declared in the Lambda CFN template — survives every push. |
| Avatar S3 bucket (dev) | `botchat-avatars-dev-253178317163` | Must have Block Public Access disabled and a public-read bucket policy. Lambda execution role has `s3:PutObject`. |
| Lambda IAM | `ssm:GetParameter` + `kms:Decrypt` on `alias/aws/ssm` | Declared in `AmplifyResourcesPolicy` in the Lambda CFN template. |

### If avatars stop generating

1. Check Lambda logs for the error — the try/catch logs failures but doesn't throw.
2. **SSM key missing or wrong:** verify `/botchat/openai-api-key` exists and the value is a valid OpenAI key.
3. **S3 permission error:** verify the Lambda execution role still has `s3:PutObject` on the bucket (can be stripped by `amplify push` if the CFN template diverges).
4. **OpenAI quota/key error:** check the OpenAI dashboard.
5. **Lambda timeout:** the pipeline takes 15–35 seconds. Lambda timeout is 90s (declared in CFN). Do not lower it.

### Adding a main-environment avatar bucket

When promoting to `main`, create a separate bucket `botchat-avatars-main-253178317163`, configure it identically (public read), and update `AVATAR_S3_BUCKET` in the Lambda's CFN template for the main stack. The SSM key at `/botchat/openai-api-key` is shared across environments — no separate key needed.
